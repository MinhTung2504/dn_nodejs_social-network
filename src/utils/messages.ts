const user_email_not_found = 'User with Email {0} not found';
const user_email_existed = 'Email already existed';
const user_error_login_credentials = 'Please check your login credentials';
const user_cannot_seflsend_friend_request =
  'It is not possible to add yourself!';
const user_cannot_resend_friend_request =
  'A friend request has already been sent of received to your account!';
const user_id_not_found = 'User with ID {0} Not Found';

const post_not_found = 'Post with ID {0} not found';
const post_deleted_successed = 'Post with ID {0} have been DELETED';

export {
  user_email_not_found,
  user_email_existed,
  user_error_login_credentials,
  post_not_found,
  post_deleted_successed,
  user_cannot_seflsend_friend_request,
  user_cannot_resend_friend_request,
  user_id_not_found,
};
